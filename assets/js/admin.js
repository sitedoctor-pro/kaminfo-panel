...
let orders = [], reviews = [], notifications = [], charts = {}, realtimeBound = false;
...
function fmtDate(s) { return new Date(s).toLocaleString("fr-MA", { dateStyle: "short", timeStyle: "short" }); }
...
async function requireAdmin() {
  ...
  if (error || !data?.length) { await sb.auth.signOut(); location.href = "login.html"; return null; }
  await logLogin(session.user); return session.user;
}

async function logLogin(user) {
  try { await sb.from("admin_login_events").insert({ user_id: user.id, email: user.email, user_agent: navigator.userAgent }); } catch (e) {}
}

function initNav() {
  qsa(".side-nav button").forEach((btn) => btn.addEventListener("click", () => {
    qsa(".side-nav button").forEach((b) => b.classList.remove("active")); btn.classList.add("active");
    qsa(".panel-section").forEach((s) => s.classList.remove("active")); qs("#" + btn.dataset.section).classList.add("active");
    qs("#sectionTitle").textContent = btn.textContent; qs("#sidebar").classList.remove("active"); qs("#sidebarToggle").classList.remove("active");
  }));
  qs("#sidebarToggle")?.addEventListener("click", (e) => { e.currentTarget.classList.toggle("active"); qs("#sidebar").classList.toggle("active"); });
...
async function loadAnalytics() {
  ...
  const [ { data: pv24 }, { data: pv7 }, { data: o30 }, { data: lToday }, { data: daily }, { data: oDaily }, { data: rDaily } ] = await Promise.all([
    sb.from("page_views").select("visitor_id").gte("created_at", since24), sb.from("page_views").select("id,visitor_id,created_at").gte("created_at", since7), sb.from("orders").select("id,total_price,created_at").gte("created_at", since30), sb.from("admin_login_events").select("id").gte("created_at", today.toISOString()), sb.from("analytics_daily").select("*").limit(14), sb.from("analytics_orders_daily").select("*").limit(14), sb.from("analytics_reviews_daily").select("*").limit(14),
  ]);
  qs("#visitors24").textContent = new Set((pv24 || []).map((x) => x.visitor_id)).size; qs("#views7d").textContent = (pv7 || []).length; qs("#orders1m").textContent = (o30 || []).length; qs("#loginsToday").textContent = (lToday || []).length;
...
function renderTraffic(rows) {
  ...
  charts.traffic = new Chart(el, { type: "line", data: { labels: rows.map((r) => r.day), datasets: [ { label: "Page Views", data: rows.map((r) => r.page_views), borderColor: "#00e8ff", backgroundColor: "rgba(0,232,255,.13)", tension: 0.42, fill: true }, { label: "Visitors", data: rows.map((r) => r.unique_visitors), borderColor: "#ff2e9b", backgroundColor: "rgba(255,46,155,.12)", tension: 0.42, fill: true } ] }, options: chartOptions() });
}
function renderOrdersChart(rows) {
  const el = qs("#ordersChart"); if (!el) return; charts.orders?.destroy();
  charts.orders = new Chart(el, { type: "bar", data: { labels: rows.map((r) => r.day), datasets: [{ label: "Orders", data: rows.map((r) => r.orders_count), backgroundColor: "rgba(143,56,255,.72)", borderRadius: 12 }] }, options: chartOptions() });
}
function renderReviewsChart(rows) {
  const el = qs("#reviewsChart"); if (!el) return; charts.reviews?.destroy();
  charts.reviews = new Chart(el, { type: "bar", data: { labels: rows.map((r) => r.day), datasets: [ { label: "Pending", data: rows.map((r) => r.pending_reviews), backgroundColor: "rgba(255,215,106,.68)", borderRadius: 12 }, { label: "Approved", data: rows.map((r) => r.approved_reviews), backgroundColor: "rgba(146,255,139,.55)", borderRadius: 12 } ] }, options: chartOptions() });
}

async function loadOrders() {
  let query = sb.from("orders").select("*").order("created_at", { ascending: false }).limit(500);
  const status = qs("#orderStatusFilter")?.value; if (status) query = query.eq("status", status);
...
function initOrders() {
  ...
  qs("#ordersTable")?.addEventListener("change", async (e) => {
    ...
    if (error) toast(error.message); else toast("Order updated");
  });
...
function initReviews() {
  qs("#reviewsAdminGrid")?.addEventListener("click", async (e) => {
    const approve = e.target.closest(".review-approve"), reject = e.target.closest(".review-reject"), del = e.target.closest(".review-delete");
