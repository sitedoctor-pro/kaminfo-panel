...
function renderTraffic(rows) {
  const el = qs("#trafficChart"); if (!el) return;
...
function renderOrdersChart(rows) {
  const el = qs("#ordersChart"); if (!el) return;
...
function renderReviewsChart(rows) {
  const el = qs("#reviewsChart"); if (!el) return;
...
async function loadNotifications() {
  ...
  if (error) return;
  notifications = data || [];
  qs("#notificationsTable").innerHTML = notifications.map((n) => `<tr><td>${fmtDate(n.created_at)}</td><td>${escapeHtml(n.event_type)}</td><td>${escapeHtml(n.title)}</td><td>${escapeHtml(n.body)}</td><td>${n.is_sent ? "Yes" : "No"}</td></tr>`).join("");
}
...
function bindRealtime() {
  ...
  sb.channel("admin-live")
    ...
    .on("postgres_changes", { event: "INSERT", schema: "public", table: "reviews" }, (payload) => {
      ...
      loadReviews();
      loadAnalytics();
      showLocalNotification("تقييم جديد ⭐", `${payload.new.customer_name} عطاك ${payload.new.rating} نجوم`);
    })
    .on("postgres_changes", { event: "INSERT", schema: "public", table: "notification_events" }, loadNotifications)
    .subscribe();
}
