...
async function loadAnalytics() {
  const since24 = new Date(Date.now() - 24 * 3600e3).toISOString(),
    ...
    since30 = new Date(Date.now() - 30 * 24 * 3600e3).toISOString();
  ...
  qs("#visitors24").textContent = new Set(
    (pv24 || []).map((x) => x.visitor_id),
  ).size;
...
function initOrders() {
  ...
  qs("#exportOrdersBtn")?.addEventListener("click", () => {
    const headers = ["الاسم", "المدينة", "العنوان", "الهاتف", "المنتج", "ملاحظات", "السعر"];
    const rows = orders.map((o) => [o.customer_name, o.city, o.address, o.phone, o.pad_choice, o.notes, o.total_price]);
...
function initReviews() {
  qs("#reviewsAdminGrid")?.addEventListener("click", async (e) => {
    ...
    if (error) toast(error.message);
    else { toast(approve ? "Review approved" : "Review rejected"); loadReviews(); loadAnalytics(); }
  });
...
// ⚠️ هادي الدالة الأصلية ديالك اللي كتخدم والتاب مفتوحة ⚠️
  ...
  sb.channel("admin-live")
    ...
    .on(
      ...
      (payload) => {
        ...
        loadReviews();
        loadAnalytics();
        showLocalNotification("تقييم جديد ⭐", `${payload.new.customer_name} submitted ${payload.new.rating} stars`);
      },
    )
...
async function enablePush() {
  try {
    ...
    if (!window.CAMINFO_VAPID_PUBLIC_KEY) throw new Error("Add your public VAPID key in assets/js/supabase-config.js first.");
    ...
    const { error } = await sb.from("push_subscriptions").upsert({
      user_id: user.id, endpoint: json.endpoint, p256dh: json.keys.p256dh, auth_secret: json.keys.auth, user_agent: navigator.userAgent, is_active: true
    }, { onConflict: "endpoint" });
...
function initPush() {
  ...
  // ⚠️ هادا السطر الأصلي ديالك اللي كيخدم السيرفيس وركر ⚠️
